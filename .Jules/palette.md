...
**Learning:** Standard comma-separated lists for metadata (like tech stacks or focus areas) can be difficult to quickly scan in plain markdown text. Wrapping individual items in inline code blocks (` \` \` `) combined with bullet separators (` • ` instead of commas or pipes) transforms them into distinct, lightweight visual "tags". This improves readability, provides better visual boundaries between items without relying on external images or complex components, and enhances the experience for screen reader users by creating clearer pauses.
